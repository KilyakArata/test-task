Мой проект микросервиса для отбора на стажировку Avito. Микросервис представляет из себя сервис по хранению и взаимодействию с "баннерами".

Описание проекта:
1. В папке cmd находится файл для запуска main.go
2. В папке config находится файл config.go, с помощью которого считывается файл config.yaml из корневой папки
3. В папке internal есть два файла: 1) internal.go, в котором расписаны все методы 2) cache.go, в котором я хочу расписать КЭШ-хранилище, для реализации 5 пункта из Условий задания, КЭШ пока не до конца реализован в internal.go
4. В папке storage находится сама база данных storage.db (на драйвере SQLite), и 2 файла: 1) storage.go, который создаёт таблицы, если они не созданы 2) app.go, который прописывает все функции работы с базой данных
5. В файле test.txt накиданы мои заготовки для тестов
6. В файле api.yaml расписано всё задание от Avito на openapi

Пока смог только реализовать все методы, без 5 и 6 задания из Условий. 3 пункт из условий реализовал как просто считывание из headers, не понимаю до конца, хотят ли они прям авторизацию, но если что базовую могу спокойно сделать. Пунктом 4 (прописыванием тестов) пока не занимался.

В дополнительные задания пока не лез.
...
## Условия
1. Используйте этот [API]([https://github.com/KilyakArata/avito-privat/blob/master/api.yaml])
